...
class Cart {
	constructor(element) {
		...
		this.i18n = {
			...
			'cart.price': 'Price',
			'cart.price-on-request': 'On request',
		...
		if (this.lang === 'de') {
			this.i18n = {
				...
				'cart.price': 'Preis',
				'cart.price-from': 'Ab',
				'cart.price-on-request': 'Auf Anfrage',
				'cart.total': 'Summe',
		...
		function createCartItem(item) {
			return `
				<tr>
					...
					<td>${item.price?.price ?? i18n['cart.price-on-request']}</td>
					<td>${item.total?.price ?? i18n['cart.price-on-request']}</td>
				</tr>
			`;
		}
			...
			taxRates.forEach((taxRate) => {
				html += `
					<tr class="text-s color-gray-500">
						...
						<td>${taxRate.price ?? i18n['cart.included-vat']}</td>
					</tr>
				`;
			});
			return html;
				...
				<table>
					...
					<tfoot>
						...
						<tr>
							...
							<td>${data.isFromPrice === true ? i18n['cart.price-from'] : ''} ${data.total?.price}</td>
						</tr>
						${createTaxRates(data.taxRates)}
					</tfoot>
				</table>
				${(this.element.dataset.variant !== 'checkout' && data.isOrderable === true) ? `<a href="${data.checkout.url}" class="button-white">${i18n['cart.to-checkout']}</a>` : ''}
			`;
